...
export default function Login() {
  ...
  return (
    <div className="app-shell" style={{ maxWidth: 420 }}>
      ...
      {/* Always rendered (visibility toggled, not mounted/unmounted) so the
          form and its Log in / Register button below never shift position
          when a message appears or disappears - that shift was causing
          clicks to land on the old button position and need a second click. */}
      <div
        className="error-banner"
        style={{
          visibility: notice || error ? "visible" : "hidden",
          ...(notice ? { background: "rgba(79,124,94,0.2)", borderColor: "var(--good)", color: "#d7ecdd" } : {}),
        }}
      >
        {notice || error || " "}
      </div>
