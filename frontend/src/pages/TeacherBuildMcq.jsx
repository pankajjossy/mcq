  ...
  async function save() {
    ...
    try {
      if (editingId) {
        ...
        // One paper, one save - even when it mixes MCQ-family questions
        // with short-answer questions, this is a single request that
        // creates a single linked paper (not two separate ones the teacher
        // has to manage independently).
        await api("/teacher/paper/save", {
          method: "POST",
          body: {
            subject,
            topic,
            semester,
            questions: draftQuestions && draftQuestions.length > 0 ? draftQuestions : undefined,
            shortQuestions: includeShort && validShort.length > 0 ? validShort : undefined,
          },
        });
      }
      navigate("/teacher");
